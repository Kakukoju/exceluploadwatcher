...
# ─── Tutti Pur Baseline (S3 Upload + Analyze → SQLite) ────────────────────────
...
# Initialize the pur_baseline DB tables on startup
init_pur_baseline_db()
...
async def tutti_analyze_content(
    ...
    """
    Analyze file content and store in EC2 SQLite.
    Routes to appropriate parser based on db_table:
      - tuttiprequn / tuttiprueqn: Equation CSV parsing
      - tuttirealassign: IDEXX xlsx parsing
      - calvalue: Tutti-計算值.xlsx parsing
    """
    content = await file.read()
